...
class OperationType(object):
    def __init__(self, idx=None,name=None, process_list = None, pt_list=None, m_list=None):
        self.idx = idx
        self.name = name
...
class Job(object):
    ...
    def __init__(self, model, env, job_type, idx, release_date, due_date):
        self.model = model
        self.env = env
        ...
        if len(self.job_type.work_list)>1:
            self.step = (-1 for w in self.job_type.work_list)
        else:
            self.step = -1
        # 현재 공정 단계를 추적하는 인덱스
    ...
    def generate_operation(self):
        ...
        """
        # 작업 유형에 정의된 공정 수와 정보를 바탕으로 작업에 필요한 'operation' 객체를 생성하고 초기화.
        for i, w_type in enumerate(self.work_list):
            for j, op_type in enumerate(w_type.op_list):
                o = Operation(self.model, self.env, idx=j, part_name=self.name,
                              op_type = op_type, requirements=None)

                self.op_list.append(o)
